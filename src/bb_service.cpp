...
void ProcessCommandNode(int i, std::vector<std::shared_ptr<BBNode>>& nodes, std::string command, std::promise<std::string>&& val, std::atomic_uint64_t& cnt) {
  nodes[i]->ExecuteCommand(nodes, command, std::move(val));
  cnt += 1;
}

void SendNotifyAll(uint64_t total_nodes, std::vector<std::shared_ptr<BBNode>>& nodes, std::atomic_uint64_t& cnt) {
...
void BBService::ProcessCommand(const std::string& command) {
